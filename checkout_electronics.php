...
if (!empty($_SESSION['electronics_cart'])) {
    mysqli_begin_transaction($conn);
    
    $totalAmount = 0;
    $itemsToInsert = [];
    
    foreach ($_SESSION['electronics_cart'] as $id => $item) {
        $product_id = (int)$id;
        $qty = $item['qty'];
        
        $check = mysqli_prepare($conn, "SELECT name, price, stock FROM electronics_table WHERE id = ?");
        ...
        if (!$product) {
            $error = true;
            $message = __('product_not_exists') . " ID $product_id";
            $messageClass = "bg-rose-500 text-white";
            break;
        }
        
        $checkOwn = mysqli_prepare($conn, "SELECT id FROM user_electronics_inventory WHERE user_id = ? AND product_id = ?");
        mysqli_stmt_bind_param($checkOwn, "ii", $user_id, $product_id);
        mysqli_stmt_execute($checkOwn);
        $ownRes = mysqli_stmt_get_result($checkOwn);
        if (mysqli_num_rows($ownRes) > 0) {
            $skipped[] = $product['name'] . " (" . __('already_owned') . ")";
        ...
        if ($product['stock'] < $qty) {
            $skipped[] = $product['name'] . " (" . __('insufficient_stock') . " - " . __('available') . ": " . $product['stock'] . ")";
            continue;
        }
    ...
    if (!$error && !empty($itemsToInsert)) {
        ...
        if (mysqli_stmt_execute($orderStmt)) {
            ...
            foreach ($itemsToInsert as $item) {
                ...
                $updateStock = mysqli_prepare($conn, "UPDATE electronics_table SET stock = ? WHERE id = ?");
                mysqli_stmt_bind_param($updateStock, "ii", $newStock, $item['id']);
                mysqli_stmt_execute($updateStock);
                mysqli_stmt_close($updateStock);
                
                $invStmt = mysqli_prepare($conn, "INSERT INTO user_electronics_inventory (user_id, product_id) VALUES (?, ?)");
    ...
    if ($error) {
        ...
        if (!empty($purchased) && !empty($skipped)) {
            $message = __('electronics_purchase_success_skipped');
            $messageClass = "bg-yellow-500 text-slate-950";
        } elseif (!empty($purchased)) {
            $message = __('electronics_purchase_success');
            $messageClass = "bg-emerald-500 text-slate-950";
        } elseif (!empty($skipped)) {
            $message = __('electronics_none_purchased');
            $messageClass = "bg-amber-500 text-slate-950";
        } else {
            $message = __('electronics_cart_empty');
            $messageClass = "bg-amber-500 text-slate-950";
        }
    }
} else {
...
<head>
    ...
    <title><?= __('electronics_checkout_title') ?> | GameStore</title>
...
<div class="content-wrapper">
    <header class="sticky top-0 z-50 bg-[#17171c] border-b border-[#2a2a30] shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <nav class="flex gap-5 text-sm">
                ...
                <a href="electronics.php" class="hover:text-cyan-400 transition"><?= __('electronics') ?></a>
                <a href="cart_electronics.php" class="hover:text-cyan-400 transition"><?= __('electronics_cart') ?></a>
    ...
    <main class="max-w-2xl mx-auto px-4 py-12">
        <div class="bg-[#17171c] rounded-xl border border-[#2a2a30] p-8 text-center fade-up">
            <div class="mb-6">
                <?php if ($success): ?>
                    <div class="text-6xl mb-4"><?= !empty($skipped) ? '⚠️' : '🎉' ?></div>
                    <h2 class="text-2xl font-bold <?= !empty($skipped) ? 'text-yellow-400' : 'text-emerald-400' ?>">
                        <?= !empty($skipped) ? __('partial_success') : __('thank_you') ?>
                    </h2>
                <?php else: ?>
            ...
            <?php if (!empty($purchased)): ?>
                <div class="mt-6 text-left bg-[#1f1f28] rounded-lg p-4">
                    <h3 class="font-semibold text-lg mb-3">✅ <?= __('purchased_items') ?>:</h3>
                    <ul class="space-y-1">
                        <?php foreach ($purchased as $name): ?>
            ...
            <?php if (!empty($skipped)): ?>
                <div class="mt-4 text-left bg-yellow-900/30 border border-yellow-600 rounded-lg p-4">
                    <h3 class="font-semibold text-yellow-300 mb-2">⚠️ <?= __('skipped_items') ?></h3>
                    <ul class="list-disc pl-5 text-yellow-100 text-sm">
            ...
            <div class="mt-8 flex justify-center gap-4">
                <a href="electronics.php" class="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-md transition transform hover:scale-105 btn-ripple"><?= __('continue_shopping') ?></a>
                <a href="cart_electronics.php" class="bg-gray-700 hover:bg-gray-600 text-white px-5 py-2 rounded-md transition"><?= __('back_to_cart') ?></a>
            </div>
        </div>
    </main>
